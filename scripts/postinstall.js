...
async function main() {
  try {
    ...
    // editable install creates a `Scripts/sace.exe` (or
    // `bin/sace`) entry-point that imports `cli.cli` from the
    // package's npm install directory. When npm later moves, prunes,
    // or upgrades that directory, the shim survives but its target
    // disappears -- the user runs `sace start` and gets
    ...
    // Detect a stale `Scripts/sace.exe` (or `bin/sace`) left
    // behind by a prior version's `pip install -e ./sace` (the dir
    // was renamed from `sace/` to `cli/`; pip's metadata can lag).
    // The pip distribution name is still ``sace`` (the public CLI
    // name); the source dir is ``cli/``. If pip shows the package
    // installed but the current `sace` on PATH is NOT our Node
    // shim, surface the cleanup command -- otherwise the user keeps
    // hitting the dead Python entry-point even after upgrading.
    try {
      const pipShow = execSync('python -m pip show sace 2>nul || python3 -m pip show sace 2>/dev/null', {
        encoding: 'utf-8', stdio: ['ignore', 'pipe', 'ignore'], shell: true,
      }).trim();
      if (pipShow) {
        console.log('');
        console.log('Notice: a previous install left `sace` registered with pip.');
        console.log('If `sace start` fails with "No module named \'sace\'",');
        console.log('clean it up once with:');
        console.log('  python -m pip uninstall -y sace');
        console.log('');
      }
    } catch {
    ...
    console.log('');
    console.log('========================================');
    console.log('  SACEOS installed successfully!');
    console.log('========================================');
    console.log('');
    console.log('Run: sace start');
