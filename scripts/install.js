...
function run(cmd, cwd = ROOT, timeoutMs = 300000) {
  ...
  // ``npm install -g saceos`` from a shell that has activated a
...
// by ``python -m services.temporal._install``, same call ``sace
// build`` already makes. The pooch cache (~/.cache/SACEOs/...)
...
// platformdirs.user_cache_path("SACEOs")/browser/npm/ on first
...
try {
  ...
  // from temporal.download/cli/archive/latest). Same call ``sace
  ...
  // 30-90 s stall on first ``sace start`` into a sub-second
  ...
  // lazily on first `sace start` — a failed fetch must never fail
  // `npm install`.
  try {
    ...
    console.log('  It will be fetched automatically on first `sace start`.');
  }
