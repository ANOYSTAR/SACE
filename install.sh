#!/usr/bin/env bash
# SACEOS Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/zeenie-ai/SACEOS/main/install.sh | bash
#
# This script installs SACEOS and its dependencies:
...
main() {
  ...
  echo ""
  info "Installing SACEOS..."
  echo ""

  # Install saceos from npm
  # On Linux/WSL without nvm, global npm install needs sudo unless prefix is user-writable
  if npm install -g saceos 2>/dev/null; then
    : # Installed successfully
  elif command -v sudo &> /dev/null; then
    info "Retrying with sudo..."
    sudo npm install -g saceos
  else
    error_exit "npm install -g failed. Try: sudo npm install -g saceos"
  fi

  echo ""
  echo -e "${GREEN}============================================${NC}"
  echo -e "${GREEN}  SACEOS installed successfully!${NC}"
  echo -e "${GREEN}============================================${NC}"
  echo ""
  echo "  Start SACEOS:"
  echo "    sace start"
