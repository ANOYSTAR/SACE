...
const COMMANDS = {
  ...
  deploy: 'Provision a cloud VM running SACEOs (Terraform)',
...
function printHelp() {
  console.log(`
SACEOS - Workflow Automation Platform

Usage: sace <command> [flags]
...
Examples:
  sace start          # Production server (clean output)
  sace start -v       # Production with full logs
  sace dev            # Development with hot-reload
  sace build          # Build for production
...
function doctor() {
  ...
  console.log(has('server/.venv/pyvenv.cfg') ? '    Python venv: OK' : '    Python venv: Missing (run sace build)');
  console.log('');
}
...
if (cmd === 'help' || cmd === '--help' || cmd === '-h') {
  ...
  console.log(`sace v${PKG.version}`);
} else if (cmd === 'doctor') {
  doctor();
