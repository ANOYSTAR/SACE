...
The CLI is **independent of the server's uv environment**: ``sace``
...
def server_venv(root: Path | None = None) -> Path:
    ...
    The CLI itself does NOT live in this venv; ``sace`` is
...
# yet -- the recovery-verb scenario (``sace clean`` against a
...
def user_data_dir() -> Path:
    ...
    ``core.paths.sace_root``. Otherwise delegates to platformdirs.
    """
