...
Project supervisor CLI for the SACEOS stack. Replaces `scripts/*.js`
...
```sh
python -m pip install -e ./sace
```

## Use

```sh
python -m sace --help        # list commands
python -m sace stop          # kill ports + orphans + temporal
```
...
- **macOS / Linux**: requires `python` to point to Python 3.11+. Most
  ...
  your distro only ships `python3`, run `python3 -m sace <cmd>` or
  add an alias.
