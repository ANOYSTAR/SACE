...
``validate_build`` refuses to launch when ``sace build`` hasn't been
...
def validate_build(root: Path, *, require_client_dist: bool = False) -> None:
    """Refuse to launch if ``sace build`` hasn't been run.
    ...
    if not node_modules_dir(root).exists() or not server_venv(root).exists():
        console.print('[red]Error: Project not built. Run "sace build" first.[/]')
        raise typer.Exit(code=1)
    if require_client_dist and not client_dist_entry(root).exists():
        console.print('[red]Error: Client not built. Run "sace build" first.[/]')
        raise typer.Exit(code=1)
