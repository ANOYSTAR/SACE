...
Stdlib-only env-var loader. Three env files, lowest precedence first:
  ...
  1. ``.env.template`` — canonical production defaults. Ships with every
     install. ``DATA_DIR=~/.sace`` (user home) is the daemon
     behaviour: ``sace start`` / ``sace daemon`` use these.
  2. ``.env`` — user overrides (created by ``scripts/postinstall.js`` on
     ``npm install -g saceos``). Gitignored.
  3. ``.env.dev`` — dev-mode overrides. Loaded ONLY by ``sace dev``
     via :func:`load_dev_overrides`. Pins ``DATA_DIR=.sace`` so
     per-checkout dev state lives at ``<repo>/.sace/`` instead of
     ``~/.sace/``. Committed to git.
...
def _require(env: dict[str, str], key: str) -> str:
    ...
    if raw is None:
        raise KeyError(
            ...
            "reinstall (``npm install -g saceos``) or restore the template."
        )
    return raw
...
class Config:
    ...
    def all_ports(self) -> list[int]:
        ...
        # on ``sace stop``. ``temporal_port`` (gRPC) and
...
def load_dev_overrides(root: Path | None = None) -> None:
    ...
    Called by ``sace dev`` BEFORE :func:`load_config` so dev-only env
    vars (notably ``DATA_DIR=.sace``, the per-checkout dev state
    ...
    (``DATA_DIR=/foo sace dev``) still wins. Missing ``.env.dev``
    file is a no-op — production-style invocations never trigger this.

    The daemon path (``sace start`` / ``sace daemon``) does not
