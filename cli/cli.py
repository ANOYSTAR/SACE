...
implementation on dispatch. Running ``sace clean`` (the recovery
...
app = typer.Typer(
    name="sace",
    help="SACEOS project supervisor CLI.",
...
@app.command(
    "stop",
    help="Stop all SACEOS services and free configured ports.",
)
def _stop() -> None:
...
@app.command(
    "serve",
    help="Run on a single public port (API + WebSocket + built SPA + sidecar). Used by `sace deploy`.",
)
def _serve(
...
daemon_app = typer.Typer(
    name="daemon",
    help="Run the SACEOs backend as a detached process.",
    no_args_is_help=True,
    add_completion=False,
)
...
deploy_app = typer.Typer(
    name="deploy",
    help="Provision a fresh cloud VM running SACEOs (via Terraform).",
    no_args_is_help=True,
    add_completion=False,
)
...
@deploy_app.command(
    "up",
    help="Create the 'saceos' VM, install SACEOs behind the login gate, and run it.",
)
def _deploy_up(
    ...
    version: str = typer.Option("latest", "--version", help="saceos version when --source release."),
...
@deploy_app.command(
    "status",
    help="Show the 'saceos' deployment's URL + health.",
)
...
@deploy_app.command(
    "destroy",
    help="Terraform-destroy the 'saceos' deployment and remove its local state.",
)
def _deploy_destroy(
