"""Cross-verb helpers shared by ``sace`` commands.
...
# top-level import list means ``sace clean`` (which calls only
...
def free_all_ports(cfg: Config) -> list[KillResult]:
    ...
    -- ``sace stop`` shows ``[OK]`` / ``[!!]`` markers and the killed
    PIDs, ``sace start`` and ``clean`` just want them gone. The
...
def build_backend_spec(
    ...
    *,
    ...
    ``sace clean`` doesn't call this and shouldn't pay for it.
    """
