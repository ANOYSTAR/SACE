"""``sace clean`` -- reset the repo to a fresh-checkout state.
...
# Only project-local artefacts. Home-rooted state (``~/.sace/``,
...
# Children of ``<repo>/.sace/`` to wipe selectively. The bare
# ``.sace`` entry can't go in ``_TARGETS`` anymore because the
...
# holds ``sace deploy``'s Terraform working dirs + state files:
# deleting state for LIVE cloud resources orphans the VM/firewall
# (``sace deploy destroy`` could no longer find them) -- only
# ``deploy destroy`` removes that tree. ``packages/`` holds the
# SACEOs-managed binaries (Temporal CLI ~114 MB, Stripe CLI, the
...
# re-download on every clean+build cycle, which hard-fails ``sace
# build`` on slow links. Anything else under ``.sace/`` (claude
...
def _kill_running_processes(cfg, root: Path) -> bool:
    ...
    try:
        ...
        from cli.ports import kill_orphaned_sace_processes
    ...
    orphaned = kill_orphaned_sace_processes(str(root), exclude_substring="-m cli")
    if orphaned:
...
def clean_command() -> None:
    ...
    # Step 4: selectively clean ``<repo>/.sace/`` -- preserve
    ...
    sace_dir = root / ".sace"
    if sace_dir.is_dir():
        for child in sace_dir.iterdir():
            if child.name in _MACHINA_KEEP:
                continue
            rel = f".sace/{child.name}"
