"""``sace dev`` -- replaces ``scripts/dev.js``.
...
def dev_command(
    ...
    # so dev state lands at ``<repo>/.sace/`` (per-checkout) instead
    # of ``~/.sace/`` (user home). Called BEFORE ``preflight()`` so
    # ``load_config``'s ``setdefault`` pass sees the dev values already
    # in ``os.environ``. ``sace start`` / ``sace daemon`` skip
