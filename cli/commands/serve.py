"""``sace serve`` -- single-port production runtime.
...
``ExecStart`` on a VM provisioned by ``sace deploy``.

Unlike ``sace start`` (which runs a separate static-client server + the
...
def serve_command(port: int | None = None) -> None:
    ...
    console.print()
    console.print("  [bold]SACEOS[/] serve (single-port)")
