"""``sace deploy up`` -- provision + run SACEOs on a fresh cloud VM.
...
The VM instance is always named ``saceos`` (one deployment per project).
"""
...
def _npm_pack(root: Path) -> str:
    ...
    if not out:
        error_block(
            ...
            ["Ensure Node/npm are installed and you are in a SACEOs checkout."],
        )
...
def up_command(
    *,
    ...
    if _state.exists() and (_state.workdir() / "terraform.tfstate").exists():
        console.print(
            "[yellow]A 'saceos' deployment already exists. Re-running will "
            "re-apply Terraform (safe), or run `sace deploy destroy` first.[/]"
        )
    ...
    console.print()
    console.print("  [bold green]VM 'saceos' provisioned.[/]")
    if ip:
    ...
    if url:
        console.log("The VM is installing SACEOs (Node + npm + build); this takes a few minutes.")
        if _poll_health(url):
            console.print(f"  [bold green]Ready.[/] Open {url} and log in.")
        else:
            console.print(
                "  [yellow]Still provisioning.[/] Check again with "
                "`sace deploy status` in a few minutes."
            )
