"""``sace deploy destroy`` -- tear down the deployment's cloud resources."""
...
def destroy_command(*, keep_state: bool = False) -> None:
    ...
    if meta is None:
        console.print("[yellow]No 'saceos' deployment found.[/]")
        raise typer.Exit(code=1)

    _terraform.ensure_terraform()
    wd = _state.workdir()

    console.log("terraform destroy (saceos)...")
    ...
    console.print("  [green]Destroyed[/] and removed local state for 'saceos'.")
