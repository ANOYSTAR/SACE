"""``sace build`` -- replaces ``scripts/build.js``.
...
expected location. Without that, ``sace build`` would pooch
Temporal under ``~/.sace/`` and ``sace dev`` would re-fetch
it into ``<repo>/.sace/`` — a redundant download on every fresh
...
# Source dirs / entry-point modules under ``server/`` that ``sace build``
...
def build_command() -> None:
    ...
    # build's ``DATA_DIR`` matches what the runtime (``sace dev``)
    # will see. Without this, ``sace build`` reads
    # ``DATA_DIR=~/.sace`` from ``.env.template`` and lands the
    # Temporal CLI under user home, but ``sace dev`` then reads
    # ``DATA_DIR=.sace`` from ``.env.dev`` and re-downloads into
    # ``<repo>/.sace/`` — a cache-miss the operator pays on every
    # fresh checkout.
    #
    # Safe for global installs: ``.env.dev`` is committed to git for
    # repo-clone contributors but is NOT in the npm ``files`` list, so
    # ``npm install -g saceos`` doesn't ship it. Without
    # ``.env.dev`` on disk, :func:`load_dev_overrides` is a no-op and
    # the build falls through to ``.env.template`` defaults
    # (``DATA_DIR=~/.sace``) — identical to today's behaviour and
    # matching what ``sace start`` / ``sace daemon`` use at
    # runtime.
    ...
    run(["pnpm", "--filter", "saceos-nodejs-executor", "run", "build"], cwd=root)
    ...
    # now so first ``sace start`` is instant instead of paying the
